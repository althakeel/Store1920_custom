...
export default function OrdersAreaChart({ chartData = [], allOrders = [] }) {
    const ordersPerDay = chartData.length
        ? chartData.reduce((acc, entry) => {
            acc[entry.date] = entry.orders;
            return acc;
        }, {})
    ...
    const resolvedChartData = chartData.length
        ? chartData
    ...
    return (
        <div className="w-full max-w-4xl h-[300px] text-xs">
            ...
            <ResponsiveContainer width="100%" height="100%"> 
                <AreaChart data={resolvedChartData}>
