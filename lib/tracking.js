...
/**
 * Single purchase entry point (order-success).
 * - GTM dataLayer `purchase` → Google Analytics / GA4 via GTM
 * - fbq Purchase with eventID → Meta Pixel (browser), deduped with server CAPI
 */
export function trackPurchase(orderInput = {}, options = {}) {
  ...
  return runTrackedOnce(purchaseKey, () => {
