...
import {
  collectOrderItemsFromImportRow,
  looksLikeOrderItemsJson,
  normalizeImportedOrderItems,
  parseEmbeddedOrderItemsJson,
  repairJsonCandidate,
} from '@/lib/importedOrderItems'
...
function parseOrderItems(value) {
  ...
  if (looksLikeOrderItemsJson(raw)) {
    return parseEmbeddedOrderItemsJson(raw)
  }
  ...
  try {
    const parsed = JSON.parse(repairJsonCandidate(raw))
...
function buildFlatOrderItem(pick) {
  ...
  if (looksLikeOrderItemsJson(name)) return null
...
function buildFallbackOrderItems(pick, total) {
  ...
  const fromJson = parseEmbeddedOrderItemsJson(productName)
  if (fromJson.length) return fromJson

  if (looksLikeOrderItemsJson(productName)) return []
...
async function importOrderRow(row, storeId) {
  ...
  if (legacySourceId && /^\d+$/.test(legacySourceId)) {
    legacySourceId = `wc-${legacySourceId}`
  }

  let customerName = normalizeText(pick.fuzzy(
    'customerName',
    'customer',
    'guestName',
    'buyername',
  ...
  if (!customerName) {
    const billingFirst = normalizeText(pick.fuzzy(
      'firstNameBilling',
      'billingFirstName',
      'firstnamebilling',
    ) || pick('firstName', 'billingFirstName'))
    const billingLast = normalizeText(pick.fuzzy(
      'lastNameBilling',
      'billingLastName',
      'lastnamebilling',
      'lastName',
    ) || pick('lastName', 'billingLastName'))
    customerName = [billingFirst, billingLast].filter(Boolean).join(' ').trim()
  }
  ...
  if (!orderItems.length) {
    orderItems = collectOrderItemsFromImportRow(pick, total)
  }
  if (!orderItems.length) {
    orderItems = buildFallbackOrderItems(pick, total)
  }
  orderItems = normalizeImportedOrderItems(orderItems)
  if (orderItems.length) {
...
export async function processImportRows(rows, storeId, { rowOffset = 0 } = {}) {
