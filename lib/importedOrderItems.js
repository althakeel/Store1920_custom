...
function hasProductReference(item = {}) {
  const productId = item?.productId;
  if (productId && typeof productId === 'object') {
    if (normalizeText(productId?.name)) return true;
    if (productId?._id || productId?.id) return true;
    // Bare Mongo ObjectId instances are objects without _id/name.
    if (typeof productId.toHexString === 'function' || typeof productId.toString === 'function') {
      const asId = String(productId);
      if (/^[a-fA-F0-9]{24}$/.test(asId)) return true;
    }
  }
  return Boolean(normalizeText(productId));
}

function isUsableItem(item = {}) {
  if (hasProductReference(item)) return true;
...
export function normalizeImportedOrderItems(items = []) {
  ...
  return normalized.filter((item) => isUsableItem(item));
}
