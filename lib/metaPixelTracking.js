...
import { hasTrackedOnce, hasTrackedPersistently, markTrackedOnce } from '@/lib/trackingDedupe';
...
export function trackPageView({ pagePath } = {}) {
  ...
  // Path only — ignore query so soft param changes do not re-fire PageView.
  const raw = String(
  ...
  // Claim before fbq so Strict Mode / GTM races cannot double-send.
  markTrackedOnce(`meta:PageView:${routeKey}`);
