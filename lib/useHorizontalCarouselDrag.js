...
export function useHorizontalCarouselDrag({
  ...
  const beginDragging = useCallback((container) => {
  ...
  const handlePointerDown = useCallback((e) => {
    ...
    try {
      ...
      // Continue with document listeners even if capture fails.
    }
  ...
  }, [attachDocumentListeners, cancelMomentum, detachDocumentListeners])
  ...
  const trackClassName = `flex ${PRODUCT_CARD_CAROUSEL_GAP_CLASS} overflow-x-auto pb-2 scrollbar-hide overscroll-x-contain snap-x snap-mandatory select-none ${
