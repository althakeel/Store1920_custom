...
export function useHorizontalCarouselDrag({
  enableSnap = true,
  enableMobileSnap = true,
  enableMomentum = true,
  enableTouchDrag = false,
  scrollStepMode = 'child',
  mobileVerticalScrollPriority = false,
  lockMobileToVerticalTouch = false,
} = {}) {
  ...
  const snapClassName = enableSnap
    ? (
      enableMobileSnap
        ? 'snap-x snap-proximity md:snap-mandatory scroll-smooth'
        : 'max-md:snap-none md:snap-x md:snap-proximity lg:snap-mandatory md:scroll-smooth'
    )
    : ''

  const touchActionClass = lockMobileToVerticalTouch
    ? 'max-md:[touch-action:pan-y] lg:touch-manipulation'
    : mobileVerticalScrollPriority
      ? 'max-md:[touch-action:pan-x_pan-y] lg:touch-manipulation'
      : 'touch-manipulation'

  const overscrollClass = lockMobileToVerticalTouch
    ? 'max-md:overscroll-y-auto max-md:overscroll-x-none lg:overscroll-x-contain'
    : 'overscroll-x-contain'

  const trackClassName = `flex ${PRODUCT_CARD_CAROUSEL_GAP_CLASS} overflow-x-auto pb-2 scrollbar-hide ${overscrollClass} ${touchActionClass} select-none ${snapClassName} ${
