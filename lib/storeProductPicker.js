const PICKER_FIELDS = '_id name sku price AED inStock createdAt images externalImages';

export function buildPickerFilter(storeId, search = '') {
...
export async function fetchPickerPage(Product, {
  ...
  const filter = buildPickerFilter(storeId, search);
...
export async function fetchPickerProductIds(Product, storeId, search = '') {
  const filter = buildPickerFilter(storeId, search);
