...
export async function sendAbandonedCheckoutWhatsApp({
  ...
  try {
    ...
    const originalTotal = Number(cartTotal ?? product?.originalPrice ?? product?.price ?? 0);
    const discountedTotal = Number(offerTotal ?? cartTotal ?? product?.price ?? originalTotal);
    const freeShippingLabel = resolveWhatsAppFreeShippingLabel(product, { cartTotal: discountedTotal });
    ...
    return await sendWhatsAppTemplate({
      ...
      bodyParams: [
        customerName || 'Customer',
        formatAedPrice(originalTotal),
        formatAedPrice(discountedTotal),
        freeShippingLabel,
        cartUrl,
      ],
      headerImageUrl: imageUrl,
