...
import { sendWhatsAppTemplate, normalizePhoneForWaba } from '@/lib/whatsapp/elasticWaba';
...
function getDisplayOrderNumber(order) {
  if (order?.shortOrderNumber) return String(order.shortOrderNumber);
  if (order?._id) return order._id.toString().slice(-8).toUpperCase();
  return 'N/A';
}
