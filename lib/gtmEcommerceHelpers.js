...
export function resolveOrderLinePackQuantity(item = {}) {
  return Math.max(1, Number(item.quantity ?? item.qty ?? 1));
}

export function resolveOrderLineQuantity(item = {}, product = null) {
  const packs = resolveOrderLinePackQuantity(item);
  const bundleQty = Number(item?.variantOptions?.bundleQty);
  if (bundleQty > 0) {
    return bundleQty * packs;
  }

  if (product) {
    const inferred = inferOrderLineBundleQty(item, product);
    if (inferred > 0) {
      return inferred * packs;
    }
  }
