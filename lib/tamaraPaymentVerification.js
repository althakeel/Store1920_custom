...
export function assertTamaraProviderOrder({
  ...
  const storedReferences = new Set(
    orders.flatMap((order) => {
      const formatted = formatPaymentProviderOrderReference(order);
      const short = normalized(order?.shortOrderNumber);
      return [
        formatted,
        short,
        short ? `ORD-${short}` : '',
  ...
  // We persist tamaraOrderId ourselves at checkout create. That binding is the
  // source of truth. Merchant references can drift off-by-one when short-order
  // numbers used to be allocated in a deferred race against session create.
  const tamaraIdOwnsGroup = orders.every(
    (order) => normalized(order?.tamaraOrderId) === expectedTamaraOrderId,
  );
  const referencesMatch = Boolean(providerReference)
    && storedReferences.has(providerReference)
    && (!webhookReference || providerReference === webhookReference);
  if (!referencesMatch && !tamaraIdOwnsGroup) {
    throw new TamaraPaymentValidationError('Tamara order reference mismatch');
  }
  if (!referencesMatch && tamaraIdOwnsGroup) {
    console.warn(
      '[tamara] order reference mismatch ignored; tamara order id owns order group',
      {
        tamaraOrderId: expectedTamaraOrderId,
        providerReference,
        webhookReference: webhookReference || null,
        shortOrderNumbers: orders.map((order) => order.shortOrderNumber),
      },
    );
  }
