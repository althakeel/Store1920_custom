...
/** Filter manage-product list by Details progress bar (e.g. 5/10, 7/10). */
export function matchesDetailsProgressFilter(product = {}, filterValue = 'all') {
  const normalized = String(filterValue || 'all').trim().toLowerCase();
  if (!normalized || normalized === 'all') return true;

  const { filledCount, totalCount } = getProductDetailCompleteness(product);
  if (normalized === 'lte5' || normalized === 'max5') return filledCount <= 5;
  if (normalized === 'lte7' || normalized === 'max7') return filledCount <= 7;
  if (normalized === 'incomplete') return filledCount < totalCount;
  return true;
}
