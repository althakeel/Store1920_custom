...
export function formatEmailLineQuantityLabel(item = {}) {
  ...
  const packQuantity = Math.max(1, Number(item.packQuantity ?? item.quantity ?? 1));

  if (item.isBulkBundle || bundleUnits > 0) {
    const units = bundleUnits || packQuantity;
    if (packQuantity > 1) {
      return `Bundle of ${units} (${packQuantity} packs)`;
    }
    return `Bundle of ${units} (1 pack)`;
  }
