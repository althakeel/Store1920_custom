...
import { runTrackedOnce, hasTrackedPersistently, markTrackedPersistently } from '@/lib/trackingDedupe';
...
export async function trackOrderSuccessPurchaseOnce(order = {}, { onAnalytics } = {}) {
  ...
  const skipBrowserMeta = Boolean(order.metaPurchaseSentAt) || hasTrackedPersistently(purchaseKey);

  // Server CAPI already sent this Purchase — skip browser fbq only (still fire TikTok/GA4).
  if (order.metaPurchaseSentAt) {
    if (process.env.NODE_ENV === 'development') {
      console.info('[Meta] Purchase skipped — already sent via CAPI', orderId);
    }
    markTrackedPersistently(purchaseKey);
  } else if (!skipBrowserMeta) {
