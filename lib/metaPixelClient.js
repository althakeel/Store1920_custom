...
import { hasTrackedOnce, markTrackedOnce, markTrackedPersistently } from '@/lib/trackingDedupe';
...
function sendMetaEventNow(eventName, params = {}, options = {}) {
  ...
  try {
    ...
    } else if (['ViewContent', 'AddToCart', 'InitiateCheckout', 'ViewCart', 'PageView'].includes(eventName)) {
      window.fbq('trackSingle', META_PIXEL_ID, eventName, payload);
    } else {
    ...
    if (eventName === 'Purchase' && options?.eventID) {
      markTrackedPersistently(`meta:Purchase:${options.eventID}`);
    }
