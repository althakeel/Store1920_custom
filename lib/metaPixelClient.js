...
import { hasTrackedOnce, markTrackedOnce, markTrackedPersistently, hasTrackedPersistently, claimPurchaseInFlight, releasePurchaseInFlight } from '@/lib/trackingDedupe';
...
export const trackMetaEvent = (eventName, params = {}, options = {}) => {
  ...
  if (!getBareFbq()) {
    if (dedupeKey) {
      ...
      // Claim immediately so Strict Mode remounts cannot enqueue a second copy
      // before the first drain completes. Drain uses force:true to still send.
      if (!isPurchase) {
        markTrackedOnce(dedupeKey);
      }
    }
