...
const HOMEPAGE_CACHE_KEY = 'server:homepage:v2';
const HOMEPAGE_TTL = 60;

const TOP_DEALS_SECTION_KEYS = new Set(['top_deals', 'top-deals', 'topdeals']);

const PRODUCT_SELECT =
  'name nameAr slug price mrp AED images category categories tags inStock stockQuantity fastDelivery freeShippingEligible imageAspectRatio averageRating ratingCount createdAt updatedAt';
...
async function fetchFeaturedSections() {
  const cacheKey = 'public:featured-sections:v2';
  const cached = getCachedData(cacheKey);
...
async function fetchFeaturedProducts(limit = 12, language = 'en') {
  ...
  let store =
    (await Store.findOne({ featuredProductIds: { $exists: true, $ne: [] } }).sort({ updatedAt: -1 }).lean()) ||
    (await Store.findOne().sort({ updatedAt: -1 }).lean());
  ...
  if (sourceMode === 'manual' && productIds.length > 0) {
    ...
    const query = store?._id ? { storeId: String(store._id) } : {};
    if (sourceMode === 'category' && Array.isArray(store?.featuredProductsCategoryIds) && store.featuredProductsCategoryIds.length) {
      query.$or = [
        { category: { $in: store.featuredProductsCategoryIds } },
        { categories: { $in: store.featuredProductsCategoryIds } },
      ];
    } else if (sourceMode === 'tag' && Array.isArray(store?.featuredProductsTags) && store.featuredProductsTags.length) {
      query.tags = { $in: store.featuredProductsTags };
    }
...
async function fetchStoreMenuSettings() {
  ...
  return {
    navMenuItems: Array.isArray(doc?.navMenuItems) ? doc.navMenuItems : [],
    navMenuStyle: doc?.navMenuStyle && typeof doc.navMenuStyle === 'object' ? doc.navMenuStyle : {},
  };
}
...
async function resolveTopDeals(sections) {
  ...
  if (!section) {
    const products = await Product.find({}).sort({ updatedAt: -1 }).select(PRODUCT_SELECT).limit(12).lean();
...
export async function getHomepageData(language = 'en') {
  const cached = getCachedData(HOMEPAGE_CACHE_KEY);
  if (cached) return cached;

  await connectDB();

  const [shopShowcase, homeSections, featuredSections, featuredProducts, appearance, storeSettings] = await Promise.all([
    fetchShopShowcase(),
    fetchHomeSections(language),
  ...
  const topDeals = await resolveTopDeals(homeSections);
  ...
  setCachedData(HOMEPAGE_CACHE_KEY, serialized, HOMEPAGE_TTL);
  return serialized;
}
