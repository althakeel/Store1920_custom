...
async function resolveTopDeals(sections) {
  ...
  if (!section) {
    const products = dedupeProductsBySku(await Product.find({ ...STOREFRONT_PUBLISHED_FILTER, inStock: { $ne: false } })
      .sort({ updatedAt: -1 })
  ...
  if (section.category) {
    const products = dedupeProductsBySku(await Product.find({
  ...
  const products = dedupeProductsBySku(await Product.find({ inStock: { $ne: false } })
    .sort({ updatedAt: -1 })
    .select(PRODUCT_SELECT)
    .limit(12)
    .lean());
