...
function mergeCustomerSummaries(existing, incoming) {
  const merged = {
    ...existing,
    name: existing.name || incoming.name,
    email: existing.email || incoming.email,
    image: isValidCustomerImage(existing.image) ? existing.image : incoming.image,
    isGuest: false,
    totalOrders: Number(existing.totalOrders || 0) + Number(incoming.totalOrders || 0),
    totalSpent: Number(existing.totalSpent || 0) + Number(incoming.totalSpent || 0),
    firstOrderDate: existing.firstOrderDate && incoming.firstOrderDate
      ? new Date(Math.min(new Date(existing.firstOrderDate).getTime(), new Date(incoming.firstOrderDate).getTime()))
      : (existing.firstOrderDate || incoming.firstOrderDate),
    lastOrderDate: existing.lastOrderDate && incoming.lastOrderDate
      ? new Date(Math.max(new Date(existing.lastOrderDate).getTime(), new Date(incoming.lastOrderDate).getTime()))
      : (existing.lastOrderDate || incoming.lastOrderDate),
    latestOrder: (() => {
      const left = existing.latestOrder;
      const right = incoming.latestOrder;
      if (!left) return right || null;
      if (!right) return left;
      return new Date(right.createdAt) >= new Date(left.createdAt) ? right : left;
    })(),
  };
...
export async function resolveRegisteredCustomerIds(customers = [], UserModel) {
  const guestEmails = [...new Set(
    customers
      .filter((customer) => customer.isGuest)
      .map((customer) => normalizeCustomerEmail(customer.email))
      .filter((email) => email && email !== 'no email' && email !== 'no email (guest)')
  )];

  if (!guestEmails.length || !UserModel) {
    return customers;
  }
  ...
  const userByEmail = new Map(
    registeredUsers.map((user) => [normalizeCustomerEmail(user.email), user])
  );
  ...
  for (const customer of customers) {
    if (customer.isGuest) {
      const registeredUser = userByEmail.get(normalizeCustomerEmail(customer.email));
      if (registeredUser) {
        ...
        const resolvedCustomer = {
          ...
          userId,
          isGuest: false,
          name: registeredUser.name || customer.name,
          email: registeredUser.email || customer.email,
          image: isValidCustomerImage(registeredUser.image) ? registeredUser.image : customer.image,
        };

        if (merged.has(userId)) {
          merged.set(userId, mergeCustomerSummaries(merged.get(userId), resolvedCustomer));
        } else {
          merged.set(userId, resolvedCustomer);
        }
        continue;
      }
    }

    const key = String(customer.id || customer._id);
    if (merged.has(key)) {
      merged.set(key, mergeCustomerSummaries(merged.get(key), customer));
    } else {
  ...
  return Array.from(merged.values()).sort(
    (left, right) => Number(right.totalSpent || 0) - Number(left.totalSpent || 0)
  );
}
