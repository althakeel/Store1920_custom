...
export async function aggregateStoreCustomers(Order, storeId, {
  ...
  const filterMatch = { $match: { $expr: { $literal: true } } };

  if (view === 'registered') {
    filterMatch.$match = { isGuest: false };
  }
      ...
      $facet: {
        ...
        filtered: [
          ...
          { $sort: { totalSpent: -1 } },
          {
            $facet: {
              total: [{ $count: 'count' }],
              customers: [{ $skip: skip }, { $limit: safeLimit }],
            },
          },
        ],
      },
  ...
  const filteredBucket = result?.filtered?.[0] || { total: [], customers: [] };
  const filteredTotal = filteredBucket.total?.[0]?.count || 0;
  const totalPages = Math.max(1, Math.ceil(filteredTotal / safeLimit));
  const currentPage = Math.min(safePage, totalPages);

  return {
    customers: filteredBucket.customers || [],
    pagination: {
      page: currentPage,
      limit: safeLimit,
      total: filteredTotal,
