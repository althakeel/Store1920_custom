...
export const STORE_ORDERS_IMPORT_START_EVENT = 'store-orders-import-start';
export const STORE_ORDERS_IMPORT_END_EVENT = 'store-orders-import-end';
export const STORE_ORDER_TOAST_ID = 'store-order-alerts';
...
export function setOrderNotificationsSuppressed(value) {
  notificationsSuppressed = Boolean(value);
}

export function isOrderNotificationsSuppressed() {
  return notificationsSuppressed;
}

export function dispatchStoreOrdersImportStart() {
  if (typeof window === 'undefined') return;
  setOrderNotificationsSuppressed(true);
  window.dispatchEvent(new CustomEvent(STORE_ORDERS_IMPORT_START_EVENT));
}

export function dispatchStoreOrdersImportEnd(detail = {}) {
  if (typeof window === 'undefined') return;
  setOrderNotificationsSuppressed(false);
  window.dispatchEvent(new CustomEvent(STORE_ORDERS_IMPORT_END_EVENT, { detail }));
}
