...
function buildVariantMatchKey(item = {}) {
  const opts = item?.variantOptions && typeof item.variantOptions === 'object' ? item.variantOptions : {};
  const parts = [];
  if (opts.bundleQty != null && opts.bundleQty !== '') {
    parts.push(`b${opts.bundleQty}`);
  }
  for (const key of ['color', 'size', 'title', 'option']) {
    const val = String(opts[key] || '').trim();
    if (val) parts.push(`${key}:${val}`);
  }
  return parts.length ? `:${parts.join(',')}` : '';
}

/** Compare cart vs order lines using pack qty (bundles) + variant options. */
export function buildProductLineSignature(items = [], context = null) {
  const lines = (Array.isArray(items) ? items : [])
    .map((item) => {
      const product = resolveOrderLineProduct(item);
      const productId = String(product?._id || item?.productId?._id || item?.productId || '').trim();
      if (!productId) return null;
      const packs = resolveOrderLinePackQuantity(item, product, context);
      return `${productId}:${packs}${buildVariantMatchKey(item)}`;
    })
    .filter(Boolean)
    .sort();
...
export function contactsMatch(cartContact, orderContact, { cart, order } = {}) {
  if (cartContact.userId && orderContact.userId && cartContact.userId === orderContact.userId) {
    return true;
  }

  const cartAnon = String(cart?.anonymousId || '').trim();
  const orderAnon = String(order?.trackingContext?.anonymousId || '').trim();
  if (cartAnon && orderAnon && cartAnon === orderAnon) {
    return true;
  }
...
export function cartMatchesPlacedOrder(cart, order) {
  ...
  const cartSignature = buildProductLineSignature(cart.items, cart);
  const orderSignature = buildProductLineSignature(order.orderItems || order.items, order);
  if (!cartSignature || !orderSignature || cartSignature !== orderSignature) {
    return false;
  }

  return contactsMatch(extractCartContact(cart), extractOrderContact(order), { cart, order });
}
