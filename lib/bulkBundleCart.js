...
export function inferOrderLineBundleQty(item = {}, product = {}) {
  ...
  if (packQty !== 1) return 0;

  const linePrice = Number(item?.price ?? 0);
  const match = bundleVariants.find(
    (variant) => Math.abs(Number(variant.price) - linePrice) < 0.02,
  );

  return match ? Number(match.options?.bundleQty) || 0 : 0;
}
