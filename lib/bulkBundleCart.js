/**
 * Bulk bundle products (Buy 1 / Buy 2 / Buy 3) use tier pricing.
 * Cart line total = tier price, not unit price × quantity.
 */

import {
  isMatrixVariant,
  getProductBundleMode,
  getMatrixBundleTiers,
  matchMatrixVariant,
} from '@/lib/productVariantOptions';

export function isBulkBundleVariant(variant) {
  if (!variant?.options) return false;
  if (isMatrixVariant(variant)) return false;
  return Boolean(
    (variant.options.bundleQty || variant.options.bundleQty === 0)
    && !variant.options?.color
    && !variant.options?.size
  );
}

export function getBulkBundleVariants(product) {
  const variants = Array.isArray(product?.variants) ? product.variants : [];
  // Bundle-only products may label rows with title ("Buy 1", etc.) — still bulk tiers.
  if (getProductBundleMode(product) === 'bulk') {
    return variants.filter((variant) => {
      const qty = Number(variant?.options?.bundleQty);
      return Number.isFinite(qty) && qty > 0;
    });
  }
  return variants.filter(isBulkBundleVariant);
}

export function getBulkBundleTiers(product) {
  const tiers = getBulkBundleVariants(product)
    .filter((v) => Number(v.stock) > 0)
    .map((v) => Number(v.options?.bundleQty) || 1);
  return [...new Set(tiers)].sort((a, b) => a - b);
}

export function isBulkBundleProduct(product) {
  return getProductBundleMode(product) === 'bulk';
}

export function isMatrixStyleProduct(product) {
  return getProductBundleMode(product) === 'matrix';
}

/** Max packs a matrix cart line can reach for the selected option + bundle tier. */
export function resolveMatrixCartMaxPacks(product, entry) {
  if (!isMatrixStyleProduct(product)) return null;
  const opts = typeof entry === 'object' ? entry?.variantOptions || {} : {};
  const tier = Number(opts?.bundleQty) || getMatrixBundleTiers(product.variants)[0] || 1;
  const variant = matchMatrixVariant(product.variants, opts, tier);
  const stock = Number(variant?.stock ?? product?.stockQuantity ?? 0);
  return Number.isFinite(stock) && stock > 0 ? stock : 0;
}

export function findBulkBundleVariant(product, tierQty) {
  const tier = Number(tierQty) || 1;
  const variants = Array.isArray(product?.variants) ? product.variants : [];
  const fromBulk = getBulkBundleVariants(product).find(
    (v) => Number(v.options?.bundleQty) === tier,
  );
  if (fromBulk) return fromBulk;
  return variants.find((v) => Number(v?.options?.bundleQty) === tier) || null;
}

export function resolveBulkCartMaxPacks(product, entry) {
  if (!isBulkBundleProduct(product)) return null;
  const tier = getBundleTierFromEntry(entry, product);
  const variant = findBulkBundleVariant(product, tier);
  const stock = Number(variant?.stock ?? product?.stockQuantity ?? 0);
  return Number.isFinite(stock) && stock > 0 ? stock : 0;
}

export function resolveBulkBundleTier(entry, product, fallbackTier) {
  if (!isBulkBundleProduct(product)) return null;

  const fromVariant = Number(entry?.variantOptions?.bundleQty);
  if (Number.isFinite(fromVariant) && fromVariant > 0) {
    return fromVariant;
  }

  const tiers = getBulkBundleTiers(product);
  if (!tiers.length) return null;

  const fromFallback = Number(fallbackTier);
  if (fromFallback && tiers.includes(fromFallback)) return fromFallback;

  return tiers[0];
}

export function getBundleTierFromEntry(entry, product) {
  return resolveBulkBundleTier(entry, product);
}

/** Map storefront quantity picker → cart pack count + bundle tier. */
export function resolveBulkBundleCartFromUiQty(uiQty, product, tiers) {
  const qty = Math.max(1, Number(uiQty) || 1);
  const bundleTiers = Array.isArray(tiers) && tiers.length
    ? tiers
    : getBulkBundleTiers(product);
  const minTier = bundleTiers[0] || 1;

  if (bundleTiers.includes(qty)) {
    return { bundleTier: qty, packQty: 1, displayQty: qty };
  }

  return { bundleTier: minTier, packQty: qty, displayQty: qty };
}

/** Units to show in the product-page / cart qty UI (bundle tier × packs, not pack count). */
export function resolveCartDisplayQuantity(product, entry, packQty, selectedTier) {
  const packs = Math.max(0, Number(packQty) || 0);
  if (!packs) return 0;

  const pricing = resolveCartLinePricing(product, entry, packs);
  const fromPricing = Number(pricing.displayQuantity);
  if (Number.isFinite(fromPricing) && fromPricing > 0) {
    return fromPricing;
  }

  const bundleTier = Number(entry?.variantOptions?.bundleQty)
    || Number(pricing.bundleTier)
    || Number(selectedTier)
    || 0;

  if (bundleTier > 0 && (isBulkBundleProduct(product) || isMatrixStyleProduct(product))) {
    return bundleTier * packs;
  }

  return packs;
}

export function resolveCartLinePricing(product, entry, qty = 1) {
  const isFreeGift = typeof entry === 'object' && entry?.freeGift;

  if (isFreeGift) {
    return {
      unitPrice: 0,
      lineTotal: 0,
      displayQuantity: qty,
      bundleTier: null,
      isBulkBundle: false,
    };
  }

  // Matrix products (color/size × bundle tier): each combination is its own
  // variant with its own price. The "unit" is one bundle, so the line total is
  // the matched variant price × number of bundles ordered.
  if (isMatrixStyleProduct(product)) {
    const matrixTiers = getMatrixBundleTiers(product?.variants);
    const priceOverride = typeof entry === 'object' ? entry?.price : undefined;
    const requestedTier = Number(entry?.variantOptions?.bundleQty);
    const tier = Number.isFinite(requestedTier) && requestedTier > 0
      ? requestedTier
      : (matrixTiers[0] || 1);
    const variant = matchMatrixVariant(product?.variants, entry?.variantOptions || {}, tier);
    const unitPrice = Number(
      variant?.price ?? priceOverride ?? product?.salePrice ?? product?.price ?? 0,
    ) || 0;
    const quantity = Math.max(1, Number(qty) || 1);
    return {
      unitPrice,
      lineTotal: unitPrice * quantity,
      // Show the bundle units (tier × packs) at checkout so a "Bundle of 3"
      // selection reflects 3 items instead of 1, matching pure bulk bundles.
      displayQuantity: tier * quantity,
      bundleTier: tier,
      isBulkBundle: false,
      isMatrix: true,
    };
  }

  if (!isBulkBundleProduct(product)) {
    const priceOverride = typeof entry === 'object' ? entry?.price : undefined;
    const unitPrice = Number(priceOverride ?? product?.salePrice ?? product?.price ?? 0) || 0;
    const quantity = Number(qty) || 0;
    return {
      unitPrice,
      lineTotal: unitPrice * quantity,
      displayQuantity: quantity,
      bundleTier: null,
      isBulkBundle: false,
    };
  }

  const tier = getBundleTierFromEntry(entry, product)
    || Number(entry?.variantOptions?.bundleQty)
    || 1;
  const variant = findBulkBundleVariant(product, tier);
  const priceOverride = typeof entry === 'object' ? entry?.price : undefined;
  const unitPrice = Number(
    variant?.price ?? priceOverride ?? product?.salePrice ?? product?.price ?? 0
  ) || 0;
  const packQty = Math.max(1, Number(qty) || 1);
  const safeTier = Math.max(1, Number(tier) || 1);

  return {
    unitPrice,
    lineTotal: unitPrice * packQty,
    displayQuantity: safeTier * packQty,
    bundleTier: safeTier,
    isBulkBundle: true,
  };
}

export function buildBundleCartEntry(entry, product, tierQty, packQty = 1) {
  const tier = Number(tierQty) || 1;
  const packs = Math.max(1, Number(packQty) || 1);
  const variant = findBulkBundleVariant(product, tier);
  const base = typeof entry === 'object' && entry !== null ? entry : {};

  return {
    ...base,
    quantity: packs,
    price: Number(variant?.price ?? base.price ?? product?.price ?? 0) || 0,
    variantOptions: {
      ...(base.variantOptions || {}),
      bundleQty: tier,
    },
  };
}

export function bundleCartSelectionMatches(cartEntry, product, selectedBundleQty) {
  if (!cartEntry || !isBulkBundleProduct(product)) return true;

  const explicitCartTier = Number(cartEntry?.variantOptions?.bundleQty);
  const selectedTier = Number(selectedBundleQty);

  if (!Number.isFinite(explicitCartTier) || explicitCartTier <= 0) {
    return false;
  }
  if (!Number.isFinite(selectedTier) || selectedTier <= 0) {
    return false;
  }

  return explicitCartTier === selectedTier;
}

export function inferOrderLineBundleQty(item = {}, product = {}) {
  const fromOptions = Number(item?.variantOptions?.bundleQty);
  if (fromOptions > 0) return fromOptions;

  const bundleVariants = getBulkBundleVariants(product);
  if (!bundleVariants.length) return 0;

  const linePrice = Number(item?.price ?? 0);
  const rawQty = Math.max(1, Number(item?.quantity ?? 1));

  const byPrice = bundleVariants.find(
    (variant) => Math.abs(Number(variant.price) - linePrice) < 0.05,
  );
  if (byPrice) return Number(byPrice.options?.bundleQty) || 0;

  const byStoredQty = bundleVariants.find(
    (variant) => Number(variant.options?.bundleQty) === rawQty,
  );
  if (byStoredQty) return rawQty;

  if (bundleVariants.length === 1) {
    return Number(bundleVariants[0].options?.bundleQty) || 0;
  }

  return 0;
}

function estimateOrderMerchandiseTotal(order = {}) {
  let total = Number(order.total || 0);
  total -= Number(order.shippingFee || 0);
  total += Number(order.walletDiscount || 0);

  const coupon = order.coupon || {};
  const discount = Number(coupon.discount || 0);
  if (discount > 0) {
    if (String(coupon.discountType || '').toLowerCase() === 'percentage') {
      if (discount < 100) {
        total = total / (1 - discount / 100);
      }
    } else {
      total += discount;
    }
  }

  return Math.max(0, total);
}

/** Last-resort bundle detect when product variants are missing on the order line. */
export function inferBundleUnitsFromOrderContext(item = {}, order = {}) {
  const rawQty = Math.max(1, Number(item?.quantity ?? 1));
  const price = Number(item?.price ?? 0);
  if (rawQty <= 1 || !price) return 0;

  const items = Array.isArray(order.orderItems) ? order.orderItems : [];
  if (items.length !== 1) return 0;

  const paidMerchandise = estimateOrderMerchandiseTotal(order);
  const multiplied = price * rawQty;

  if (multiplied <= paidMerchandise * 1.25) return 0;
  if (Math.abs(price - paidMerchandise) > price * 0.2) return 0;

  return rawQty;
}

export function adjustBundleCartTier(entry, product, direction) {
  if (!isBulkBundleProduct(product)) return null;

  const tiers = getBulkBundleTiers(product);
  const current = getBundleTierFromEntry(entry, product);
  const idx = tiers.indexOf(current);

  if (direction === 'up') {
    if (idx < 0 || idx >= tiers.length - 1) return null;
    return buildBundleCartEntry(entry, product, tiers[idx + 1]);
  }

  if (direction === 'down') {
    if (idx <= 0) return 'remove';
    return buildBundleCartEntry(entry, product, tiers[idx - 1]);
  }

  return null;
}
