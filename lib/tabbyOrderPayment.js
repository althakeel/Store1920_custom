...
export function validateTabbyPaymentForOrders(payment, orders, {
  ...
  // We persist tabbyPaymentId ourselves at checkout create. That binding is the
  // source of truth. Order references can drift when short-order-number
  // allocation races deferred assignment (ORD-{n+1} on Tabby vs {n} in Mongo).
  const paymentIdOwnsGroup = orderGroup.every((order) => (
    String(order?.tabbyPaymentId || '').trim() === requestedPaymentId
  ));
  const referencesMatch = Boolean(parsed.orderId)
    && tabbyOrderGroupReferences(orderGroup).has(parsed.orderId);
  if (!referencesMatch && !paymentIdOwnsGroup) {
    return { valid: false, reason: 'tabby_order_reference_mismatch', parsed };
  }
  if (!referencesMatch && paymentIdOwnsGroup) {
    console.warn(
      '[tabby] order reference mismatch ignored; payment id owns order group',
      {
        paymentId: requestedPaymentId,
        providerReference: parsed.orderId,
        orderIds: orderGroup.map((order) => String(order._id)),
        shortOrderNumbers: orderGroup.map((order) => order.shortOrderNumber),
      },
    );
  }
