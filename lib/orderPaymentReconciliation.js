...
import { verifyStripeOrderPayment as verifyStripeOrderPaymentById } from '@/lib/stripeOrderPayment';
...
async function verifyStripeOrderPayment(order, stripeIndex) {
  ...
  if (fallback?.success && !fallback?.alreadyPaid) {
    return { fixed: true, provider: 'STRIPE' };
  }
...
async function verifyTamaraOrderPayment(order) {
  ...
  try {
    await captureTamaraPayment(tamaraOrderId, {
      orderId: formatPaymentProviderOrderReference(populated || order) || orderId,
      amount: order.total,
...
export async function reconcileStoreOrderPayments(storeId, {
  ...
  const candidates = await Order.find({
    ...
    .select('_id shortOrderNumber paymentMethod paymentStatus status isPaid tabbyPaymentId tamaraOrderId razorpayPaymentId stripeCheckoutSessionId total createdAt')
    .sort({ createdAt: -1 })
