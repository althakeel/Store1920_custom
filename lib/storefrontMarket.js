...
export function formatDisplayAmount(value, marketCode = DEFAULT_STOREFRONT_MARKET, language = 'en', options = {}) {
  const {
    minimumFractionDigits = 2,
    maximumFractionDigits = 2,
  } = options;

  return formatLocalizedNumber(value, marketCode, language, {
    minimumFractionDigits,
    maximumFractionDigits,
  });
}

export function formatSplitPriceParts(value, marketCode = DEFAULT_STOREFRONT_MARKET, language = 'en') {
  const numeric = Number(value || 0);
  ...
  const locale = getStorefrontLocale(marketCode, language);
  const formatOptions = language === 'ar'
    ? { minimumFractionDigits: 2, maximumFractionDigits: 2, ...ARABIC_NUMBER_FORMAT_OPTIONS }
    : { minimumFractionDigits: 2, maximumFractionDigits: 2 };

  const parts = new Intl.NumberFormat(locale, formatOptions).formatToParts(numeric);
  let integerPart = '';
  let decimalPart = '00';
