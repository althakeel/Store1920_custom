...
export async function trackPickup(bookingNo) {
  ...
  const payloads = [
    { PickupRequestNo: value, ...creds },
    { BookingNo: value, ...creds },
    { PickupNo: value, ...creds },
    { TrackingAWB: value, ...creds },
  ]
  const errors = []

  for (const payload of payloads) {
    try {
      return await c3xPost('PickupTracking', payload)
    } catch (error) {
      errors.push(error?.message || String(error))
    }
  }

  throw new Error(errors[0] || 'C3Xpress PickupTracking failed')
}
