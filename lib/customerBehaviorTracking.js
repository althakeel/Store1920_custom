...
export async function resolveCustomerIdentity({
  ...
  const resolvedAnonymousId = isPresent(anonymousId) ? String(anonymousId).trim() : null;
  ...
  const identifier = {
    ...
    source: resolvedAnonymousId && !resolvedFirebaseUid && !resolvedUserId && !picked.hasAny
      ? 'anonymous_guest'
      : picked.source,
    fallbackNote: resolvedAnonymousId && !resolvedFirebaseUid && !resolvedUserId && !picked.hasAny
      ? 'Guest tracked via anonymous browser ID'
      : picked.source === 'explicit_payload'
...
export function shouldDropForMissingIdentity(identifier) {
  ...
  return !(
    ...
    isPresent(identifier.anonymousId)
  );
}
