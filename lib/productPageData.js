...
const RELATED_SELECT =
  'name nameAr slug price AED images category categories inStock stockQuantity imageAspectRatio averageRating ratingCount';

const FBT_SELECT =
  'name nameAr price images slug hasVariants variants inStock stockQuantity';
...
async function fetchReviewsForProduct(productId) {
  ...
  const reviews = await Rating.find({
    ...
    .sort({ createdAt: -1 })
    .limit(30)
    .select('rating review images videos customerName customerEmail userId createdAt')
    .lean();
