...
function inferUploadsBaseUrl(state) {
  for (const post of state.posts.values()) {
    if (String(post.post_type || '') !== 'attachment') continue
    const guid = normalizeText(post.guid)
    const marker = '/wp-content/uploads/'
    const markerIndex = guid.indexOf(marker)
    if (markerIndex > 0) {
      return `${guid.slice(0, markerIndex)}${marker}`
    }
  }
...
function resolveAttachmentUrl(attachmentId, state, uploadsBaseUrl = '') {
  const normalizedId = String(attachmentId || '').trim()
  if (!normalizedId) return ''

  const attachmentPost = state.posts.get(normalizedId)
  const guid = normalizeText(attachmentPost?.guid)
  if (/^https?:\/\//i.test(guid)) {
    return guid
  }

  const attachmentMeta = state.postMeta.get(normalizedId) || {}
  const attachedFile = normalizeText(attachmentMeta._wp_attached_file)
  if (!attachedFile) return ''

  if (/^https?:\/\//i.test(attachedFile)) {
    return attachedFile
  }

  if (uploadsBaseUrl) {
    const safeBase = uploadsBaseUrl.replace(/\/+$/, '')
    const safePath = attachedFile.replace(/^\/+/, '')
...
function extractAttachmentIds(metaValue) {
  const values = ensureArray(metaValue)
  const ids = []
...
function resolveProductImages(meta = {}, state, uploadsBaseUrl = '') {
  const attachmentIds = [
    ...extractAttachmentIds(meta._thumbnail_id),
    ...extractAttachmentIds(meta._product_image_gallery),
  ]

  const orderedIds = [...new Set(attachmentIds)]
  const images = orderedIds
    .map((attachmentId) => resolveAttachmentUrl(attachmentId, state, uploadsBaseUrl))
    .filter(Boolean)
...
async function importProductsFromSql(state, storeIdString, categoryMap, counts) {
  ...
  const uploadsBaseUrl = inferUploadsBaseUrl(state)
  ...
  for (const post of posts) {
    ...
    const images = resolveProductImages(meta, state, uploadsBaseUrl)
...
async function saveImportedOrder(order, counts) {
  const preserveDates = Boolean(order.createdAt)
  await order.save({ timestamps: !preserveDates })

  if (!order.shortOrderNumber && order._id) {
    order.shortOrderNumber = await allocateShortOrderNumber(order.storeId)
    await order.save({ timestamps: !preserveDates })
  }
...
async function importLegacyPostOrders(state, storeIdString, userMap, productMap, counts) {
  ...
  for (const post of posts) {
    ...
    const orderDate = parseDate(post.post_date_gmt || post.post_date)
    if (orderDate) {
...
async function importHposOrders(state, storeIdString, userMap, productMap, counts) {
  ...
  for (const hposOrder of orders) {
    ...
    const orderDate = parseDate(hposOrder.date_created_gmt || hposOrder.date_created)
    if (orderDate) {
      order.createdAt = orderDate
      order.updatedAt = orderDate
    }
...
export async function runSqlStoreImport(settings, storeId) {
  ...
  if (settings.entitySelection?.media) {
    warnings.push('Full media-library import is not mapped yet. Product images are linked from WordPress attachment metadata when available.')
  }
