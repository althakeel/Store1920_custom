import { STORE1920_LOGO_URL, emailLogoImg } from '@/lib/brandLogo';
import { withEmbeddedEmailLogo, getEmailLogoResendAttachment } from '@/lib/brandLogoEmail';
...
function sendOrderWaitingForPickupEmail({ email, name, order }) {
  const subject = `Waiting for Pickup - ${orderNoLabel(order)}`;
  const html = `
    <div style="background:#fbbf24;padding:32px 0;text-align:center;color:#fff;">
      <img src="${STORE1920_LOGO_URL}" style="max-width:200px;margin-bottom:16px;"/>
    ...
    <div style="padding:32px;background:#f8f9fa;">
      ...
      <p style="margin-top:30px;">Order No: <b>${orderNoLabel(order)}</b></p>
    </div>
  `;
...
function sendOrderOutForDeliveryEmail({ email, name, order }) {
  const subject = `Out for Delivery - ${orderNoLabel(order)}`;
  const html = `
...
function sendOrderReturnRequestedEmail({ email, name, order }) {
  const subject = `Return Requested - ${orderNoLabel(order)}`;
  const html = `
...
export async function sendOrderConfirmedEmail({ email, name, order }) {
  const subject = `Order Confirmed - ${orderNoLabel(order)}`;
  const html = `
...
export async function sendOrderPickupRequestedEmail({ email, name, order }) {
  const subject = `Pickup Requested - ${orderNoLabel(order)}`;
  const html = `
    <div style="background:#2874f0;padding:32px 0;text-align:center;color:#fff;">
      <img src="${STORE1920_LOGO_URL}" style="max-width:200px;margin-bottom:16px;"/>
...
export async function sendOrderPickedUpEmail({ email, name, order }) {
  const subject = `Order Picked Up - ${orderNoLabel(order)}`;
  const html = `
    <div style="background:#2874f0;padding:32px 0;text-align:center;color:#fff;">
      <img src="${STORE1920_LOGO_URL}" style="max-width:200px;margin-bottom:16px;"/>
...
export async function sendOrderWarehouseReceivedEmail({ email, name, order }) {
  const subject = `Order Received at Warehouse - ${orderNoLabel(order)}`;
  const html = `
    <div style="background:#2874f0;padding:32px 0;text-align:center;color:#fff;">
      <img src="${STORE1920_LOGO_URL}" style="max-width:200px;margin-bottom:16px;"/>
...
export async function sendOrderCustomStatusEmail({ email, name, order, status }) {
  const subject = `Order Update (${status}) - ${orderNoLabel(order)}`;
  const html = `
    <div style="background:#2874f0;padding:32px 0;text-align:center;color:#fff;">
      <img src="${STORE1920_LOGO_URL}" style="max-width:200px;margin-bottom:16px;"/>
...
export async function sendOrderProcessingEmail({ email, name, order }) {
  const subject = `Order Processing - ${orderNoLabel(order)}`;
  const html = `
    <div style="background:#2874f0;padding:32px 0;text-align:center;color:#fff;">
      <img src="${STORE1920_LOGO_URL}" style="max-width:200px;margin-bottom:16px;"/>
...
export async function sendOrderDeliveredEmail({ email, name, order }) {
  const subject = `Order Delivered - ${orderNoLabel(order)}`;
  const html = `
    <div style="background:#2874f0;padding:32px 0;text-align:center;color:#fff;">
      <img src="${STORE1920_LOGO_URL}" style="max-width:200px;margin-bottom:16px;"/>
    ...
    <div style="padding:32px;background:#f8f9fa;">
      <h2>Hi ${name || 'there'}!</h2>
      <p>Thank you for shopping with Store1920. If you have any feedback or need support, reply to this email.</p>
      <p style="margin-top:30px;">Order No: <b>${orderNoLabel(order)}</b></p>
    </div>
  `;
  return sendMail({ to: email, subject, html, storeId: order?.storeId });
...
export async function sendOrderCancelledEmail({ email, name, order }) {
  const subject = `Order Cancelled - ${orderNoLabel(order)}`;
  const html = `
    <div style="background:#2874f0;padding:32px 0;text-align:center;color:#fff;">
      <img src="${STORE1920_LOGO_URL}" style="max-width:200px;margin-bottom:16px;"/>
...
export async function sendOrderReturnedEmail({ email, name, order }) {
  const subject = `Order Returned - ${orderNoLabel(order)}`;
  const html = `
    <div style="background:#2874f0;padding:32px 0;text-align:center;color:#fff;">
      <img src="${STORE1920_LOGO_URL}" style="max-width:200px;margin-bottom:16px;"/>
...
async function sendViaPlatformSmtp({ to, subject, html, headers, fromType, attachments = [] }) {
 ...
 */
export async function sendMail({ to, subject, html, tags, headers, fromType = 'transactional', storeId, skipStoreSmtp = false }) {
  const { html: emailHtml, attachments: logoAttachments } = withEmbeddedEmailLogo(html);
  const resendLogoAttachment = getEmailLogoResendAttachment();
  ...
  if (prefersPlatformSmtp()) {
    try {
      const info = await sendViaPlatformSmtp({
        to, subject, html: emailHtml, headers, fromType, attachments: logoAttachments,
      });
      if (info) return info;
    } catch (error) {
  ...
  if (resend) {
    try {
      ...
      if (resendLogoAttachment) {
        emailPayload.attachments = [resendLogoAttachment];
      }
      ...
      <div class="container">
        ...
        <div class="content">
          ...
          <div class="support-section">
            <p>💬 <strong>Need help?</strong> Our support team is here for you! Contact us at <a href="mailto:support@Store1920.com">support@Store1920.com</a></p>
          </div>
        </div>
        <div class="footer">
...
export async function sendOrderConfirmationEmail(orderData) {
  ...
  const html = `
    ...
    <body>
      <div class="container">
        <div class="header">
          <img src="${STORE1920_LOGO_URL}" alt="Store1920 Logo" class="header-logo" />
          <h1>✅ Order Confirmed!</h1>
        ...
        <div class="content">
          ...
          <div class="order-id">
            ...
            <span style="color: #6b7280; font-size: 14px;">Placed on ${new Date(createdAt).toLocaleDateString('en-AE', { dateStyle: 'long' })}</span>
          </div>
          ...
          ` : ''}
          <center>
            <a href="https://Store1920.com/track-order" class="button" style="background:#3b82f6;color:#fff;padding:14px 36px;text-decoration:none;border-radius:8px;font-weight:bold;font-size:1rem;letter-spacing:0.5px;box-shadow:0 2px 8px rgba(40,116,240,0.08);transition:background 0.2s;">Track Order</a>
          </center>
  ...
  `;
  return sendMail({ to: email, subject, html, storeId: orderData?.storeId });
}
    ...
    <body>
      <div class="container">
        ...
        <div class="content">
          ...
          <center>
            <a href="https://Store1920.com/track-order" class="button" style="background:#3b82f6;color:#fff;padding:12px 30px;text-decoration:none;border-radius:8px;font-weight:bold;display:inline-block;">Track Order</a>
          </center>
  ...
  const html = `
    ...
    <body>
      <div class="container">
        ...
        <div class="footer">
          <p style="margin-bottom: 0;">Need help? Contact us at support@Store1920.com</p>
          <p>© ${new Date().getFullYear()} Store1920. All rights reserved.</p>
        </div>
      </div>
    </body>
  ...
  const paymentSection = paymentLink
    ? `
      <div style="text-align: center; margin: 28px 0;">
        <a href="${paymentLink}" style="display: inline-block; background: #059669; color: #fff; text-decoration: none; padding: 14px 28px; border-radius: 8px; font-weight: 700;">
    ...
    `
    : `
      <p style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 14px; color: #334155;">
        Payment method: <strong>${paymentMethodLabel || 'Cash on delivery'}</strong><br>
  ...
  const html = `
    ...
    <head>
      ...
      <style>
        ...
        .container { max-width: 620px; margin: 24px auto; background: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 2px 16px rgba(15, 23, 42, 0.08); }
        .header { background: #0f172a; color: #fff; padding: 28px 24px; text-align: center; }
        .content { padding: 28px 24px; }
        .footer { background: #f8fafc; border-top: 1px solid #e2e8f0; padding: 18px 24px; text-align: center; color: #64748b; font-size: 13px; }
        table { width: 100%; border-collapse: collapse; margin-top: 16px; }
        th { text-align: left; padding: 12px; background: #f8fafc; border-bottom: 1px solid #e2e8f0; font-size: 13px; color: #64748b; }
      </style>
    </head>
    <body>
      <div class="container">
        <div class="header">
          ${emailLogoImg('max-width:180px;height:auto;margin-bottom:12px;')}
          <h1 style="margin: 0; font-size: 24px;">${safeStoreName}</h1>
          <p style="margin: 8px 0 0; opacity: 0.85;">Complete your abandoned cart</p>
        </div>
        <div class="content">
          <p>Hi ${safeName},</p>
          <p>We saved your cart and prepared your order for <strong>${formattedAmount}</strong>.</p>
          ${paymentSection}
          ${itemsHtml ? `
            <h3 style="margin-top: 28px; margin-bottom: 8px; font-size: 16px;">Items in your cart</h3>
            <table>
              <thead>
                <tr>
                  <th>Product</th>
        ...
        <div class="footer">
          <p style="margin: 0;">© ${new Date().getFullYear()} ${safeStoreName}</p>
        </div>
      </div>
    </body>
    </html>
...
export async function sendAbandonedCartRecoveryLinkEmail({
  ...
  const safeName = String(customerName || 'there').trim() || 'there';
  const safeStoreName = String(storeName || 'Store1920').trim() || 'Store1920';
  const formattedOriginal = `${currency} ${Number(originalTotal || 0).toLocaleString(undefined, {
    minimumFractionDigits: 0,
    maximumFractionDigits: 2,
  })}`;
  const formattedOffer = `${currency} ${Number(offerTotal || 0).toLocaleString(undefined, {
    minimumFractionDigits: 0,
    maximumFractionDigits: 2,
  })}`;
  ...
  const itemsHtml = Array.isArray(items) && items.length
    ? items.map((item) => {
      ...
      const offerUnit = Number(item?.price ?? originalUnit);
      const originalLine = originalUnit * qty;
      const offerLine = offerUnit * qty;
      const priceCell = originalLine > offerLine + 0.001
        ? `<span style="text-decoration: line-through; color: #94a3b8; margin-right: 8px;">${currency} ${originalLine.toFixed(2)}</span><strong style="color: #059669;">${currency} ${offerLine.toFixed(2)}</strong>`
        : `<strong>${currency} ${offerLine.toFixed(2)}</strong>`;
      return `
      <tr>
        <td style="padding: 12px; border-bottom: 1px solid #e5e7eb;">${item?.name || 'Product'}</td>
    ...
    <body>
      <div class="container">
        <div class="header">
          ${emailLogoImg('max-width:180px;height:auto;margin-bottom:12px;')}
          <h1 style="margin: 0; font-size: 24px;">${safeStoreName}</h1>
          <p style="margin: 8px 0 0; opacity: 0.85;">A special offer on your saved cart</p>
        </div>
        <div class="content">
          <p>Hi ${safeName},</p>
          <p>We saved your cart and prepared a private offer for you.</p>
          <p style="background: #ecfdf5; border: 1px solid #a7f3d0; border-radius: 8px; padding: 14px; color: #065f46;">
            <span style="text-decoration: line-through; color: #64748b;">${formattedOriginal}</span>
            <strong style="font-size: 20px; margin-left: 8px;">${formattedOffer}</strong>
            ${discountLabel ? `<br><span style="font-size: 14px;">${discountLabel}</span>` : ''}
          </p>
          <div style="text-align: center; margin: 28px 0;">
            <a href="${recoveryLink}" style="display: inline-block; background: #059669; color: #fff; text-decoration: none; padding: 14px 28px; border-radius: 8px; font-weight: 700;">
              View your discounted cart
            </a>
          </div>
          <p style="color: #64748b; font-size: 14px; word-break: break-all;">
            Or copy this private link: <a href="${recoveryLink}">${recoveryLink}</a>
          </p>
          ${itemsHtml ? `
            <h3 style="margin-top: 28px; margin-bottom: 8px; font-size: 16px;">Items in your cart</h3>
            <table>
          ...
          ` : ''}
          <p style="margin-top: 24px; color: #64748b; font-size: 14px;">
            This link is private. Only you can see the discounted prices shown there.
          </p>
        </div>
        <div class="footer">
          <p style="margin: 0;">© ${new Date().getFullYear()} ${safeStoreName}</p>
  ...
  const html = `
    ...
    <head>
      ...
      <style>
        body { font-family: Arial, sans-serif; line-height: 1.6; color: #1e293b; background: #f8fafc; margin: 0; padding: 0; }
        .container { max-width: 620px; margin: 24px auto; background: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 2px 16px rgba(15, 23, 42, 0.08); }
        .header { background: linear-gradient(135deg, #f97316 0%, #ea580c 100%); color: #fff; padding: 28px 24px; text-align: center; }
        .content { padding: 28px 24px; }
        .alert { background: #fff7ed; border: 1px solid #fdba74; border-radius: 8px; padding: 14px; color: #9a3412; margin: 18px 0; }
        .footer { background: #f8fafc; border-top: 1px solid #e2e8f0; padding: 18px 24px; text-align: center; color: #64748b; font-size: 13px; }
        table { width: 100%; border-collapse: collapse; margin-top: 16px; }
        th { text-align: left; padding: 12px; background: #f8fafc; border-bottom: 1px solid #e2e8f0; font-size: 13px; color: #64748b; }
      </style>
    ...
    <body>
      <div class="container">
        <div class="header">
          ${emailLogoImg('max-width:180px;height:auto;margin-bottom:12px;')}
          <h1 style="margin: 0; font-size: 24px;">Order not completed</h1>
          <p style="margin: 8px 0 0; opacity: 0.9;">We saved your cart for you</p>
        </div>
        <div class="content">
          <p>Hi ${safeName},</p>
          <p>Your payment at <strong>${safeStoreName}</strong> was not completed, so your order was <strong>not placed</strong>.</p>
          <div class="alert">
            <strong>Reason:</strong> ${safeReason}
          </div>
          <p>Good news — your items are still waiting for you. Complete checkout now for <strong>${formattedAmount}</strong>.</p>
          <div style="text-align: center; margin: 28px 0;">
            <a href="${checkoutLink}" style="display: inline-block; background: #ea580c; color: #fff; text-decoration: none; padding: 14px 28px; border-radius: 8px; font-weight: 700;">
              Complete your order
            </a>
          </div>
          ${itemsHtml ? `
          ...
          ` : ''}
          <p style="margin-top: 24px; color: #64748b; font-size: 14px;">
            You have not been charged. If you need help, reply to this email and our team will assist you.
          </p>
        </div>
        <div class="footer">
          <p style="margin: 0;">© ${new Date().getFullYear()} ${safeStoreName}</p>
