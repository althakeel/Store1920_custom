...
async function sendStyledStatusEmail({ email, name, order, subject, title, intro, bodyHtml = '' }) {
  ...
  const detailsHtml = [
    ...
    order?.total != null
      ? renderEmailTotals([{ label: 'Total', value: order.total, isTotal: true }], { currency })
      : '',
    shippingAddress
      ? renderEmailAddressColumns({
        billing: buildAddressBlock({ ...shippingAddress, email: order?.guestEmail || order?.email }, name),
  ...
  const html = await buildTransactionalEmail({
    ...
    greeting: name || 'there',
    intro,
    orderNo: orderNoLabel(order),
    orderDate: formatEmailDate(order?.createdAt),
    bodyHtml: detailsHtml,
  });
  return sendOrderMail({ to: email, subject, html, storeId: order?.storeId });
}
...
export async function sendOrderPlacedEmail(orderData) {
  ...
  const addressBlock = buildAddressBlock(
    { ...shippingAddress, email: shippingAddress?.email || email },
    name,
  );

  const bodyHtml = [
    renderEmailItemsList(items, { currency, label: 'ITEMS ORDERED' }),
    renderEmailTotals([
      { label: 'Total', value: total || 0, isTotal: true },
    ], { currency }),
    paymentMethod
      ? `<div style="margin-top:24px;font-size:14px;color:#666666;">Payment method: <strong>${paymentMethod}</strong></div>`
      : '',
    renderEmailAddressColumns({
...
export async function sendAdminNewOrderEmail(order = {}) {
  ...
  const customerName = order.guestName || shipping.name || order.userId?.name || 'Customer';
  const customerEmail = order.guestEmail || shipping.email || order.userId?.email || '—';
  const customerPhone = [
    shipping.phoneCode || order.alternatePhoneCode,
    order.guestPhone || shipping.phone || order.alternatePhone,
  ].filter(Boolean).join(' ') || '—';
