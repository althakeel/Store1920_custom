...
export async function sendOrderStatusEmail(order, status) {
  const { trackingId, trackingUrl, courier } = order;
  ...
  switch (status) {
    ...
    case 'WAREHOUSE_RECEIVED':
      ...
      break;
    case 'SHIPPED':
      result = await sendOrderShippedEmail({ email, name, orderId: order._id, shortOrderNumber: order.shortOrderNumber, trackingId, trackingUrl, courier });
      break;
