 ...
 */
export async function sendMail({ to, subject, html, tags, headers, fromType = 'transactional', storeId }) {
  const resolvedFrom = (() => {
  ...
  const storeSmtpSettings = await getStoreSmtpSettings(storeId, fromType);
  ...
  if (resend) {
    ...
    try {
      ...
      const fromName = process.env.EMAIL_FROM_NAME || 'Store1920';
