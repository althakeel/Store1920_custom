...
export async function sendMail({ to, subject, html, tags, headers, fromType = 'transactional', storeId, skipStoreSmtp = false }) {
  ...
  if (smtpTransporter) {
    try {
      const fallbackFrom = parseFromAddress(process.env.SMTP_USER || resolvedFrom);
      const info = await smtpTransporter.sendMail({
        from: `${fallbackFrom.name} <${fallbackFrom.email}>`,
        to,
        subject,
        html,
      });
...
export async function sendAbandonedCartConversionEmail({
  ...
  return sendMail({ to: recipient, subject, html, storeId });
}
