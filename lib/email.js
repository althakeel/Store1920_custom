...
async function sendOrderWaitingForPickupEmail({ email, name, order }) {
  ...
  return sendStyledStatusEmail({
    ...
    order,
    subject: packed
      ? `Order Packed — Awaiting Pickup - ${orderNoLabel(order)}`
      : `Waiting for Pickup - ${orderNoLabel(order)}`,
    title: packed ? 'Order Packed — Awaiting Pickup' : 'Waiting for Pickup',
    intro: packed
      ? 'Great news — your order has been packed and is awaiting pickup by our delivery partner. We will notify you as soon as it is picked up from our warehouse.'
...
export async function sendOrderShippedEmail(orderData) {
  ...
  const bodyHtml = [
    ...
    trackingUrl
      ...
      : renderEmailCta({
        label: 'Track Order',
        href: buildTrackOrderPageUrl(orderData),
        align: 'center',
      }),
  ].filter(Boolean).join('');
...
export async function sendAdminDailyOrdersDigestEmail({ window = {}, summary = {} } = {}) {
  ...
  const orderRows = (summary.orders || []).map((row) => {
    const bucketColor = row.bucket === 'failed'
      ? '#b45309'
    ...
    return `
    <tr>
      <td style="padding:10px 12px;border-bottom:1px solid #eee;font-size:13px;color:#111;font-weight:600;">#${escapeHtml(row.orderNumber || '—')}</td>
      <td style="padding:10px 12px;border-bottom:1px solid #eee;font-size:12px;color:#444;">
        ${escapeHtml(row.orderDate || '—')}<br/>
        <span style="color:#888;">${escapeHtml(row.orderTime || '')}</span>
      </td>
      <td style="padding:10px 12px;border-bottom:1px solid #eee;font-size:13px;color:#111;">${escapeHtml(row.customerName || '—')}</td>
      <td style="padding:10px 12px;border-bottom:1px solid #eee;font-size:12px;color:#444;">${escapeHtml(row.paymentMethod || '—')}</td>
      <td style="padding:10px 12px;border-bottom:1px solid #eee;font-size:12px;color:${bucketColor};font-weight:600;">${escapeHtml(row.statusLabel || row.status || '—')}</td>
      <td style="padding:10px 12px;border-bottom:1px solid #eee;font-size:13px;color:#111;text-align:right;font-weight:600;">${escapeHtml(digestMoney(row.total))}</td>
    </tr>
  `;
  }).join('');

  const bodyHtml = [
    `<p style="margin:0 0 16px;font-size:14px;line-height:1.7;color:#444444;">Daily orders digest for <strong>${periodLabel}</strong>.</p>`,
      ...
      ? `<div style="margin:0 0 8px;font-size:11px;letter-spacing:0.14em;text-transform:uppercase;color:#9a9a9a;font-weight:600;">By status</div>
         <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 24px;border:1px solid #e8e8e8;border-collapse:collapse;">
           <tr style="background:#fafafa;">
             ...
             <th align="right" style="padding:10px 12px;font-size:11px;letter-spacing:0.08em;text-transform:uppercase;color:#888;">Orders</th>
             <th align="right" style="padding:10px 12px;font-size:11px;letter-spacing:0.08em;text-transform:uppercase;color:#888;">Total</th>
           </tr>
           ${statusRows}
         </table>`
      : `<p style="margin:0 0 20px;font-size:14px;color:#666;">No orders in this period.</p>`,
    orderRows
      ? `<div style="margin:0 0 8px;font-size:11px;letter-spacing:0.14em;text-transform:uppercase;color:#9a9a9a;font-weight:600;">Orders</div>
         <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 12px;border:1px solid #e8e8e8;border-collapse:collapse;">
           <tr style="background:#fafafa;">
             <th align="left" style="padding:10px 12px;font-size:11px;letter-spacing:0.08em;text-transform:uppercase;color:#888;">Order</th>
             <th align="left" style="padding:10px 12px;font-size:11px;letter-spacing:0.08em;text-transform:uppercase;color:#888;">Date</th>
             <th align="left" style="padding:10px 12px;font-size:11px;letter-spacing:0.08em;text-transform:uppercase;color:#888;">Customer</th>
             <th align="left" style="padding:10px 12px;font-size:11px;letter-spacing:0.08em;text-transform:uppercase;color:#888;">Payment</th>
             <th align="left" style="padding:10px 12px;font-size:11px;letter-spacing:0.08em;text-transform:uppercase;color:#888;">Status</th>
             <th align="right" style="padding:10px 12px;font-size:11px;letter-spacing:0.08em;text-transform:uppercase;color:#888;">Total</th>
           </tr>
           ${orderRows}
         </table>
         ${summary.truncated ? `<p style="margin:0 0 20px;font-size:12px;color:#888;">Showing first ${summary.orders.length} of ${summary.totalOrders} orders.</p>` : ''}`
      : '',
    renderEmailCta({
  ...
  const html = await buildTransactionalEmail({
    ...
    greeting: 'Team',
    intro: `Orders from ${window.label || 'the last business day'} (Asia/Dubai).`,
    bodyHtml,
