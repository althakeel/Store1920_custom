...
const smtpHost = String(process.env.SMTP_HOST || 'mail.store1920.com').trim();
...
function getPlatformSmtpCredentials(fromType) {
  if (isMarketingFromType(fromType)) {
    const user = String(process.env.SMTP_PROMO_USER || process.env.SMTP_USER || '').trim();
    const pass = String(process.env.SMTP_PROMO_PASS || process.env.SMTP_PASS || '').trim();
...
async function sendViaPlatformSmtp({ to, subject, html, headers, fromType }) {
...
export async function sendMail({ to, subject, html, tags, headers, fromType = 'transactional', storeId, skipStoreSmtp = false }) {
  ...
  if (prefersPlatformSmtp()) {
    try {
      const info = await sendViaPlatformSmtp({ to, subject, html, headers, fromType });
      if (info) return info;
    } catch (error) {
