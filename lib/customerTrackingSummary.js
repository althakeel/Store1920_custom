...
export function buildSessionVisitorMap(events = [], identityLinks = new Map()) {
...
export function getVisitorKey(event = {}, sessionToVisitorKey = new Map(), identityLinks = new Map()) {
  const sessionId = event.context?.sessionId;
  ...
  events.forEach((event) => {
    ...
    switch (event.eventType) {
      case 'page_view':
        ...
        break;
      case 'product_view':
      case 'product_view_ping':
        {
          const productLabel =
            event.context?.metadata?.productName ||
            event.context?.metadata?.productSlug ||
            extractProductSlugFromPath(event.context?.pagePath) ||
            event.context?.productId ||
            event.context?.pagePath;
          uniquePush(visitor.productViews, productLabel);
          uniquePush(session.productViews, productLabel);
        }
        break;
      case 'click':
        visitor.clicks += 1;
