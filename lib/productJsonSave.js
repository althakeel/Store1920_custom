...
function resolveVariantPricing(body = {}) {
  ...
  if (hasVariants) {
    ...
    inStock = stocks.some((s) => s > 0) || productStockFallback > 0;
    ...
    inStock = productStockFallback > 0;
  }
...
export async function updateProductFromJson(body, storeId) {
  ...
  const productStockFallback = body.stockQuantity !== undefined
    ? Number(body.stockQuantity) || 0
    : Number(product.stockQuantity) || 0;

  if (hasVariants) {
    variants = Array.isArray(body.variants) ? body.variants : [];
    // If matrix/variant rows were saved with 0 stock but product Stock Qty is set,
    // backfill so the storefront does not show Out of stock incorrectly.
    if (productStockFallback > 0) {
      variants = variants.map((variant) => {
        const stock = Number(variant?.stock);
        if (Number.isFinite(stock) && stock > 0) return variant;
